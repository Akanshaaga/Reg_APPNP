...
if __name__ == '__main__':
    ...
    model = DAGNN(k=12, in_dim=features.shape[1], hid_dim=64, out_dim=labels.max().item() + 1)
    ...
    filename = "dagnn_biased_rslt.txt"
    # Open the file in append mode
    with open("dagnn_biased_rslt.txt", "a") as file:
        for run in range(1):
    ...
    appnp_cmd = np.loadtxt("dagnn_biased_rslt.txt")
