...
def KMM(X, Xtest, _A=None, _sigma=1e1, beta=0.2):
    ...
    sol = solvers.qp(matrix(H.numpy().astype(np.double)), matrix(f.numpy().astype(np.double)), matrix(G), matrix(h),
                     matrix(_A), matrix(b))
    # sol = solvers.qp(matrix(H.numpy().astype(np.double)),
    #                  matrix(f.numpy().astype(np.double)))
...
if __name__ == '__main__':
    ...
    filename = "srgnn_cmd_rslt.txt"
    # Open the file in append mode
    with open("srgnn_cmd_rslt.txt", "a") as file:
        for run in range(1):
    ...
    appnp_cmd = np.loadtxt("srgnn_cmd_rslt.txt")
