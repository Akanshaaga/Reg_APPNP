...
def MMD(X, Xtest):
    H = torch.exp(- 1e0 * pairwise_distances(X)) + torch.exp(- 1e-1 * pairwise_distances(X)) + torch.exp(
        - 1e-3 * pairwise_distances(X))+ torch.exp(- 1e-5 * pairwise_distances(X))
    f = torch.exp(- 1e0 * pairwise_distances(X, Xtest)) + torch.exp(- 1e-1 * pairwise_distances(X, Xtest)) + torch.exp(
        - 1e-3 * pairwise_distances(X, Xtest))+ torch.exp(- 1e-5 * pairwise_distances(X, Xtest))
    z = torch.exp(- 1e0 * pairwise_distances(Xtest, Xtest)) + torch.exp(
        - 1e-1 * pairwise_distances(Xtest, Xtest)) + torch.exp(- 1e-3 * pairwise_distances(Xtest, Xtest)) \
        + torch.exp(- 1e-5 * pairwise_distances(Xtest, Xtest))
    MMD_dist = H.mean() - 2 * f.mean() + z.mean()
...
if __name__ == '__main__':
    ...
    model = APPNP(g, features.shape[1], [32*3], labels.max().item() + 1, F.relu, feat_drop=0.5,
    ...
    for epoch in range(EPOCH):
        ...
        if METHOD == 'SRGNN':
            ...
            loss = loss.mean() + 0.1*model.shift_robust_output(idx_train, iid_train) \
                   + 1*model.MMD_output(idx_train, iid_train)
        elif METHOD == 'DANN':
            ...
            loss = loss.mean() + 0.5*model.MMD_output(idx_train, iid_train)
        elif METHOD is None:
            loss = loss.mean()
        loss.backward()
        optimiser.step()
    ...
    filename = "appnp_citeseer.txt"
    # Open the file in append mode
    with open("appnp_citeseer.txt", "a") as file:
        for run in range(1):
    ...
    appnp_cmd = np.loadtxt("appnp_citeseer.txt")

    data = np.array(appnp_cmd)
    standard_deviation = np.std(data) #/ np.sqrt(len(data))
    print("Average accuracy:", data.mean(), "Standard Error:", standard_deviation, "n_run:", len(data))
    '''
