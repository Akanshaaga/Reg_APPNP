...
if __name__ == '__main__':
    ...
    model = APPNP(g, features.shape[1], [32*4], labels.max().item() + 1, F.relu, feat_drop=0.5, edge_drop=0.5, alpha=0.1, k=12)
    ...
    filename = "appnp_rslt.txt"
    # Open the file in append mode
    with open("appnp_rslt.txt", "a") as file:
        for run in range(1):
    ...
    appnp_cmd = np.loadtxt("appnp_rslt.txt")
